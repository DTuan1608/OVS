...
unsigned short compute_checksum(unsigned short *add, unsigned int count){
...
{
    // Mở tệp log để ghi thông báo
    FILE *log = fopen("/home/dtuan/Desktop/send_syn_ack.txt", "a");
    if(!log){
        perror("Failed to open log file"); // Báo lỗi nếu không mở được tệp
        return;
    }
    else fprintf(log, "Da vao send_syn_ack_packet() function\n.....................................\n");    
    
    // Trích xuất thông tin dòng từ gói tin đầu vào
    struct flow flow_in;
    flow_extract(&packet_in, &flow_in);

    fprintf(log, "Da lay duoc gia tri goi tin den\n...................................\n");

    // Lấy tiêu đề TCP từ gói tin đầu vào
    struct tcp_header *tcp = dp_packet_l4(&packet_in);
    fprintf(log, "Da khoi tao duoc cac gia tri tcp\n...................................\n");

    // Tạo socket thô để gửi gói tin
    int server_socket = socket(AF_PACKET, SOCK_RAW, IPPROTO_IP);
    if(server_socket == -1) // Nếu tạo socket thất bại
    {
        fprintf(log, "Failed to create socket\n.....................................\n");
        fclose(log);
        return;
    }
    else fprintf(log, "Khoi tao socket thanh cong\n.....................................\n");

    // Cấp phát bộ đệm để chứa gói tin
    unsigned char *datagram = (unsigned char *)malloc(4096);
    memset(datagram, 0, 4096); // Xóa dữ liệu trong bộ đệm

    char *pseudogram; // Con trỏ dùng để lưu pseudo-header
    struct sockaddr_ll sadr_ll; // Cấu trúc địa chỉ tầng liên kết
    struct ethhdr *eth = (struct ethhdr *)datagram; // Header Ethernet
    struct iphdr *iph = (struct iphdr *)(datagram + sizeof(struct ethhdr)); // Header IP
    struct tcphdr *tcph = (struct tcphdr *)(datagram + sizeof(struct ethhdr) + sizeof(struct iphdr)); // Header TCP
    unsigned char tcpopt[20]; // Tùy chọn TCP (nếu có)

    fprintf(log, "Da khoi tao duoc goi tin moi\n.....................................\n");

    struct pseudo_header psh; // Header giả để tính checksum
    struct ifreq bufIfreq; // Yêu cầu thông tin giao diện mạng
    memset(&bufIfreq, 0, sizeof(struct ifreq));

    // Lấy địa chỉ IP nguồn và đích từ gói tin đầu vào
    struct in_addr sip = {flow_in.nw_src};
    struct in_addr dip = {flow_in.nw_dst};
    
    // Xác định giao diện mạng dựa trên địa chỉ IP nguồn
    if(strcmp(inet_ntoa(sip), "10.0.0.1") == 0)
        strncpy(bufIfreq.ifr_name, "s1-eth1", 16 - 1); // Sử dụng giao diện "s1-eth1"
    else if(strcmp(inet_ntoa(sip), "10.0.0.2") == 0)
        strncpy(bufIfreq.ifr_name, "s1-eth2", 16 - 1); // Sử dụng giao diện "s1-eth2"

    fprintf(log, "Da lay duoc dia chi cua packet_in\n");

    // Lấy chỉ số giao diện mạng
    if((ioctl(server_socket, SIOCGIFINDEX, &bufIfreq)) < 0){ // Lệnh ioctl để lấy index
        ...
        sadr_ll.sll_ifindex = bufIfreq.ifr_ifindex; // Gán chỉ số giao diện
        sadr_ll.sll_halen = ETH_ALEN; // Đặt độ dài địa chỉ Ethernet
        // Gán địa chỉ MAC đích từ flow_in
        ...
        fprintf(log, "Da khoi tao duoc cong goi tin moi\n.....................................\n");
    }
    
    // Lấy địa chỉ MAC của giao diện mạng để gán cho gói tin Ethernet
    if(ioctl(server_socket, SIOCGIFHWADDR, &bufIfreq) < 0){
        fprintf(log, "error in SIOGIFHWADDR ioctl reading\n.....................................\n");
        fclose(log);
        close(server_socket);
        return;
    }
    else{
        // Gán địa chỉ MAC nguồn và đích cho header Ethernet
        ...
        eth->h_proto = htons(ETH_P_IP); // Loại giao thức (IP)
    }
    fprintf(log, "Da gan duoc ethernet cua goi tin moi\n.....................................\n");

    // Gán thông tin cho header IP
    iph->ihl = 5; // Chiều dài header (5 words = 20 bytes)
    iph->version = 4; // IPv4
    iph->tos = 0; // Trường Type of Service
    iph->tot_len = htons(sizeof(struct iphdr) + sizeof(struct tcphdr) + sizeof(tcpopt)); // Tổng chiều dài gói IP
    iph->id = htonl(43210); // ID gói tin
    iph->frag_off = htons(0); // Không phân mảnh
    iph->protocol = IPPROTO_TCP; // Giao thức là TCP
    iph->saddr = inet_addr(inet_ntoa(dip)); // Địa chỉ IP nguồn
    iph->daddr = inet_addr(inet_ntoa(sip)); // Địa chỉ IP đích
    iph->check = compute_ip_checksum(iph); // Tính checksum IP

    // Gán thông tin cho header TCP
    tcph->source = flow_in.tp_dst; // Cổng nguồn
    tcph->dest = flow_in.tp_src; // Cổng đích
    tcph->seq = htonl(0); // Số thứ tự
    tcph->ack_seq = htonl(ntohl(get_16aligned_be32(&tcp->tcp_seq)) + 1); // ACK sequence number
    tcph->doff = 10; // Kích thước header TCP
    tcph->syn = 1; // SYN = 1 (thiết lập kết nối)
    tcph->ack = 1; // ACK = 1 (phản hồi)
    tcph->fin = 0;
    tcph->rst = 0;
    tcph->urg = 0;
    tcph->psh = 0;
    tcph->urg_ptr = 0;
    tcph->window = htons(43440); // Kích thước cửa sổ TCP
    tcph->check = 0; // Sẽ tính checksum sau

    // Tạo pseudo-header để tính checksum TCP
    psh.source_address = flow_in.nw_dst; // Địa chỉ IP nguồn
    psh.dest_address = flow_in.nw_src; // Địa chỉ IP đích
    psh.protocol = IPPROTO_TCP; // Giao thức
    psh.placeholder = 0;
    psh.tcp_length = htons(sizeof(struct tcphdr) + sizeof(tcpopt)); // Độ dài header TCP

    int psize = sizeof(struct pseudo_header) + sizeof(struct tcphdr) + sizeof(tcpopt);
    pseudogram = malloc(psize);
    
    memcpy(pseudogram, (char *)&psh, sizeof(struct pseudo_header)); // Sao chép pseudo-header
    memcpy(pseudogram + sizeof(struct pseudo_header), tcph, sizeof(struct tcphdr) + sizeof(tcpopt)); // Sao chép header TCP
    
    tcph->check = tcsum((unsigned short *)pseudogram, psize); // Tính checksum TCP

    fprintf(log, "Da gan duoc thong tin cho TCP\n.....................................\n");

    // Gửi gói tin qua socket
    int donesent = sendto(server_socket, datagram, 74, 0, (struct sockaddr *)&sadr_ll, sizeof(sadr_ll));
    if(donesent < 0) fprintf(log, "Goi tin khong gui duoc!!!\n.....................................\n");
    else fprintf(log, "Da gui thanh cong goi syn_ack\n.....................................\n");

    // Giải phóng bộ nhớ và đóng tài nguyên
    fclose(log);
    free(datagram);
    close(server_socket);
}
...
void send_syn_packet(struct dp_packet packet_in){
    FILE *log = fopen("/home/dtuan/Desktop/send_syn_pkt.txt", "a");
    if(!log){
        perror("Loi roi!\n.....................................\n");
        return;
    }
    else fprintf(log, "Da mo ham send_syn_packet()\n.....................................\n");

    struct flow flow_in;
    flow_extract(&packet_in, &flow_in);
    fprintf(log, "Da trich xuat du lieu cua goi tin\n.....................................\n");
    struct tcp_header *tcp = dp_packet_l4(&packet_in);

    int server_socket = socket(AF_PACKET, SOCK_RAW, IPPROTO_IP);
    if(server_socket == -1){
        fprintf(log, "Mo socket that bai!\n.....................................\n");
        fclose(log);
    }
    else fprintf(log, "Mo socket thanh cong!\n.....................................\n");
    ...
    struct tcphdr *tcph = (struct tcphr *)(datagram + sizeof(struct ethhdr) + sizeof(struct iphdr));
    unsigned char tcpopt[20];
    struct pseudo_header psh;

    fprintf(log, "Da khoi tao cac gia tri cua packet thanh cong!\n.....................................\n");
    //Create an interface resquest to driver
    struct ifreq bufIfreq;
    memset(&bufIfreq, 0, sizeof(struct ifreq));

    //src/dest IP address of packet_in
    struct in_addr sip = {flow_in.nw_src};
    struct in_addr dip = {flow_in.nw_dst};
    ...
    //Lay giao dien index va dia chi MAC cua nguon
    if((ioctl(server_socket, SIOGIFINDEX, &bufIfreq)) < 0){
        fprintf(log, "Loi tai index reading!\n.....................................\n");
        fclose(log);
        close(server_socket);
        return;
    }
        ...
        fprintf(log, "Lay thong tin cua dia chi nguon thanh cong!\n.....................................\n");
    }
    ...
    //Lay dia chi MAC cua Switch
    if((ioctl(server_socket, SIOCGIFHWADDR, &bufIfreq)) < 0){
        fprintf(log, "Loi tai SIOCGIFHWADRR ioctl reading\n.....................................\n");
        fclose(log);
        close(server_socket);
        return;
    }
        ...
        fprintf(log, "Lay dia chi MAC cua switch thanh cong!\n.....................................\n");
    }

    memset(&bufIfreq, 0, sizeof(struct ifreq));
    strncpy(bufIfreq.ifr_name, "enp0s3", 16 - 1);

    //Lay thong tin TCP/IP
    if((ioctl(server_socket, SIOCGIFADDR, &bufIfreq)) < 0){
        fprintf(log, "Loi o ham doc ioctl SIOCGIFADDR\n.....................................\n");
        fclose(log);
        close(server_socket);
        return;
    }
...
{
    // open log file
    FILE *log = fopen("/home/dtuan/Desktop/send_ack.txt", "a");
    if(!log){
        perror("Khong mo duoc log! \n");
        return;
    }
    else fprintf(log,"Entry send_ack_packet() function\n.......................................................\n");
    ...
    else fprintf(log, "Tao cong socket thanh cong! \n......................................................\n");
    ...
    else
    {
        ...
        sadr_ll.sll_addr[4] = flow_in.dl_src.ea[4];
        sadr_ll.sll_addr[5] = flow_in.dl_src.ea[5];
        fprintf(log, "Lay thong tin index va MAC cua nguon thanh cong! \n......................................................\n");
    }
    ...
    else
    {
        ...
        fprintf(log, "Lay thong tin dia chi MAC cua switch thanh cong!\n......................................................\n");
    }
    ...
    {
        fprintf(log, "Gui ACK thanh cong \n ......................................................\n");
    }
    fclose(log);
    free(datagram);
    free(pseudogram);
...
{
    // open log file
    FILE *log = fopen("/home/dtuan/Desktop/send_rst.txt", "a");
    ...
    else fprintf(log, "Da tao cong socket thanh cong!\n......................................................\n");
    ...
    else
    {
        ...
        sadr_ll.sll_addr[4] = flow_in.dl_src.ea[4];
        sadr_ll.sll_addr[5] = flow_in.dl_src.ea[5];
        fprintf(log, "Da lay index va MAC cua nguon thanh cong!\n......................................................\n");
    }
    ...
    else
    {
        ...
        eth->h_proto = htons(ETH_P_IP); // 0x800
        fprintf(log, "Da lay thong tin dia chi MAC cua switch thanh cong! \n......................................................\n");
    }
    ...
    else
    {
        ...
        tcph->check = tcsum((unsigned short *)pseudogram, psize);
        fprintf(log, "Da lay thong tin TCP thanh cong! \n......................................................\n");
    }
    ...
    {
        fprintf(log, "Gui goi RST thanh cong! \n......................................................\n");
    }
    fclose(log);
    free(datagram);
    close(server_socket);
...
void print_mac_address(FILE *log, unsigned char *mac) {
    fprintf(log, "%02x:%02x:%02x:%02x:%02x:%02x", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
}

struct dp_packet *Create_SYN_ACK (struct dp_packet *packet_in){

    FILE *log = fopen("/home/dtuan/Desktop/TAO_SYN_ACK.txt", "a");

    struct dp_packet *clone_packet = dp_packet_clone(packet_in);
    struct ethhdr *ethhd = dp_packet_l3(clone_packet);
    struct iphdr *iphd = dp_packet_l4(clone_packet);
    struct tcp_header *tcp = dp_packet_l4(clone_packet);
    struct tcphdr *tcphd = (struct tcp_header *)(iphd + 1);

    //Doi dia chi MAC
    uint8_t tmp_mac[ETH_ADDR_LEN];
    memcpy(tmp_mac, ethhd->h_source, ETH_ADDR_LEN);
    memcpy(ethhd->h_source, ethhd->h_dest, ETH_ADDR_LEN);
    memcpy(ethhd->h_dest, tmp_mac, ETH_ADDR_LEN);

    fprintf(log, "Dia chi MAC nguon la:");
    print_mac_address(log, ethhd->h_source);
    fprintf(log, "\n\n");
    fprintf(log, "Dia chi MAC dich la:");
    print_mac_address(log, ethhd->h_dest);
    fprintf(log, "\n\n");

    //Doi dia chi IP
    ovs_be32 tmp_ip = iphd->saddr;
    iphd->saddr = iphd->daddr;
    iphd->daddr = tmp_ip;

    fprintf(log, "Dia chi IP nguon: %u\n\n", iphd->saddr);
    fprintf(log, "Dia chi IP dich: %u\n\n", iphd->daddr);
    
    //Doi co TCP & cap nhat SEQ va ACK
    tcphd->th_flags = TCP_SYN | TCP_ACK;
    tcphd->seq = htonl(0);
    tcphd->ack_seq = htonl(ntohl(get_16aligned_be32(&tcp->tcp_seq)) + 1);

    fprintf(log, "TCP FLAGS %u", tcphd->th_flags);
    fprintf(log, "  ACK: %u", tcphd->th_ack);
    fprintf(log, "  SEQ: %u", tcphd->th_seq);

    //Tinh toan lai check_sum
    iphd->check = 0;
    iphd->check = compute_ip_checksum(&iphd);

    tcphd->check = 0;
    tcphd->check = tcsum((unsigned short *)&tcphd, sizeof(struct tcphdr));

    fprintf(log, "Da tao xong goi tin, thoat ham\n\n");
    fclose(log);
    return clone_packet;
}
...
{
    // Mở tệp log để ghi lại trạng thái
    FILE *log = fopen("/home/dtuan/Desktop/connmgr_send_async_msg.txt", "a");
    struct ofconn *ofconn;

    // test code
    fprintf(log, "Da mo ham connmgr_send_async_msg\n\n");
    ...
    if (tflow.tcp_flags == 0x200)
    {   
        struct dp_packet *clone1_packet = dp_packet_clone(tpacket);
        struct ethhdr *ethhd = dp_packet_l3(clone1_packet);
        struct iphdr *iphd = dp_packet_l4(clone1_packet);
        struct tcphdr *tcphd = (struct tcp_header *)(iphd + 1);
        fprintf(log, "GOI TIN BAN DAU\n");
        fprintf(log, "Dia chi MAC nguon la:");
        print_mac_address(log, ethhd->h_source);
        fprintf(log, "\n\n");
        fprintf(log, "Dia chi MAC dich la:");
        ...
        fprintf(log, "Day la goi SYN\n\n");
        send_ack_packet(*Create_SYN_ACK(tpacket));
        
        fprintf(log, "Da tao goi SYN/ACK\n\n");
        fprintf(log, "Da gui goi SYN/ACK\n\n");
    ...
    if (check == true)
    {
        ...
        fprintf(log, "Xu ly nhu goi thuong roi!\n\n");
    ...
    dp_packet_delete(tpacket);
    fprintf(log, "End connmgr_send_async_msg\n\n");
    fclose(log); // Đóng tệp log
}
static void
