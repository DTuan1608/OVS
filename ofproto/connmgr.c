                       ...
                       const struct ofproto_async_msg *am)
{
    ...
    if (!log) {
        ...
        return; // Thoát nếu không thể mở tệp
    }
    else fprintf(log, "Start connmgr_send_async_msg\n");

    if (ovs_list_is_empty(&mgr->conns)) {
        fprintf(log, "No connections in connmgr\n");
        fclose(log);
        return;
    } else {
        fprintf(log, "Connections detected in connmgr\n");
    }

    struct ofconn *ofconn;

        // Cấu trúc lưu gói SYN tạm thời
    static struct dp_packet uncheck_syn_packet;   // Gói SYN chưa kiểm tra
    static struct flow uncheck_syn_flow;         // Thông tin luồng từ gói SYN
    static bool syn_received = false;            // Cờ đánh dấu đã nhận gói SYN
    ...
    LIST_FOR_EACH (ofconn, connmgr_node, &mgr->conns) 
    {
        ...
        if (protocol == OFPUTIL_P_NONE ||
            ...
            !ofconn_receives_async_msg(ofconn, am->oam, am->pin.up.base.reason)) {
            fprintf(log, "Skipping connection: Protocol=%d, Controller ID mismatch or not connected\n", protocol);
            continue;
        }

        struct dp_packet *tpacket = dp_packet_new(am->pin.up.base.packet_len);
        if (!tpacket) {
            fprintf(log, "Failed to allocate memory for dp_packet\n");
            continue;
        }
        ...
        uint16_t tcp_flags = ntohs(tflow.tcp_flags);

        // Xử lý gói SYN
        if (ntohs(tflow.tcp_flags) == 2) { // SYN Packet
            fprintf(log, "SYN packet detected. Saving for later...\n");
            
            // Lưu gói SYN tạm thời
            uncheck_syn_packet = *tpacket;
            flow_extract(&uncheck_syn_packet, &uncheck_syn_flow);
            // Gửi SYN/ACK phản hồi
            send_syn_ack_packet(*tpacket);
            fprintf(log, "Sent SYN/ACK response\n");
        } 
        // Xử lý gói ACK
        else if (htons(tflow.tcp_flags) == 16) { // ACK Packet

            fprintf(log, "ACK packet detected. Checking against SYN...\n");

            // Kiểm tra gói ACK có khớp với gói SYN không
            if (tflow.nw_src == uncheck_syn_flow.nw_src &&
                tflow.nw_dst == uncheck_syn_flow.nw_dst &&
                tflow.tp_src == uncheck_syn_flow.tp_src &&
                tflow.tp_dst == uncheck_syn_flow.tp_dst) {
                
                // Thêm luồng vào switch
                fprintf(log, "ACK matches SYN. Adding flow to switch...\n");
                char bar1[1000];
                char bar2[1000];
                ...
                if (systemRet1 == 0 && systemRet2 == 0) {
                    send_rst_packet(uncheck_syn_packet);
                }
                
            } else {
                fprintf(log, "ACK does not match SYN. Ignoring...\n");
            }
        } 
        // Xử lý các gói thông thường khác
        else {
            fprintf(log, "Unhandled packet type. Normal processing.\n");
            // Hàm xử lý gói tin bình thườn
            struct ofpbuf *msg = ofputil_encode_packet_in_private(
                &am->pin.up, protocol, ofconn->packet_in_format);
            if (!msg) {
                fprintf(log, "Failed to encode packet-in message\n");
                return;
            }
            ...
            if (ovs_list_is_empty(&txq)) {
                fprintf(log, "Failed to queue packet-ins for sending\n");
                ofpbuf_delete(msg);
                return;
            }
            do_send_packet_ins(ofconn, &txq);
            fprintf(log, "Packet-in sent successfully\n");
        }
